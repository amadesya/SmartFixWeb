...
export const Tabs: React.FC<TabsProps> = ({ options, activeTab, onTabChange, className }) => {
    return (
        <div className={`${className} flex overflow-x-auto no-scrollbar border-b border-gray-200 dark:border-smartfix-medium/20 mb-1 gap-1 bg-gray-100 dark:bg-smartfix-darker/50 p-1 rounded-xl`}>
            {options.map((option) => (
                <button
                    key={option.id}
                    ...
                    className={`px-3 md:px-4 py-1.5 md:py-2 text-xs md:text-sm font-bold transition-all whitespace-nowrap rounded-lg ${activeTab === option.id
                            ? 'bg-emerald-600 text-white dark:bg-smartfix-medium dark:text-smartfix-lightest shadow-md'
